...
export default function GamePage() {
  ...
  // BUG FIX: счётчик вместо булева toggle — гарантирует срабатывание useEffect
  // в SaveIndicator даже при нескольких последовательных сохранениях
  const [saveCount, setSaveCount] = useState(0)
  const [pageLoading, setPageLoading] = useState(true)

  const typingTimer = useRef(null)
  const fullText = useRef('')
  // BUG FIX: храним isTyping в ref чтобы избежать stale closure в startTyping
  const isTypingRef = useRef(false)

  // BUG FIX: startTyping вынесен в useCallback чтобы fetchScene мог ссылаться
  // на актуальную версию без stale closure
  ...
  const handleChoice = useCallback(async (choice) => {
    ...
    // Сохраняем прогресс
    // BUG FIX: передаём current_scene (snake_case) — совпадает с сигнатурой saveProgress
    await saveProgress(telegramId, updatedProgress)

    // BUG FIX: счётчик вместо toggle — SaveIndicator всегда получит новое значение
    setSaveCount(c => c + 1)
